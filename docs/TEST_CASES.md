...
워커 서비스의 헬스체크 엔드포인트를 검증하는 테스트입니다. (포트 4001)
...
> **테스트 파일**: `apps/api/src/health/health.controller.spec.ts`

API 서버의 헬스체크 엔드포인트를 검증하는 테스트입니다. (포트 4000)

---

#### TC-API-HEALTH-001: 헬스체크 기본 응답

| 항목 | 내용 |
|------|------|
| **테스트 ID** | TC-API-HEALTH-001 |
| **테스트명** | API 헬스체크 기본 응답 검증 |
| **우선순위** | Critical |
...
#### TC-API-HEALTH-002: 큐 상태 카운트 검증

| 항목 | 내용 |
|------|------|
| **테스트 ID** | TC-API-HEALTH-002 |
| **테스트명** | API 큐별 작업 카운트 검증 |
| **우선순위** | High |
...
#### TC-API-HEALTH-003: Readiness 체크 (Redis 연결)

| 항목 | 내용 |
|------|------|
| **테스트 ID** | TC-API-HEALTH-003 |
| **테스트명** | API Readiness 체크 - Redis 연결 확인 |
| **우선순위** | Critical |
| **엔드포인트** | `GET /api/health/ready` |
...
#### TC-API-HEALTH-004: Liveness 체크

| 항목 | 내용 |
|------|------|
| **테스트 ID** | TC-API-HEALTH-004 |
| **테스트명** | API Liveness 체크 |
| **우선순위** | High |
| **엔드포인트** | `GET /api/health/live` |
...
#### TC-API-HEALTH-005: 빈 큐 처리

| 항목 | 내용 |
|------|------|
| **테스트 ID** | TC-API-HEALTH-005 |
| **테스트명** | API 빈 큐 상태 처리 |
| **우선순위** | Medium |
...
#### TC-API-HEALTH-006: 대량 작업 카운트 처리

| 항목 | 내용 |
|------|------|
| **테스트 ID** | TC-API-HEALTH-006 |
| **테스트명** | API 대량 작업 카운트 처리 |
| **우선순위** | Medium |
...
### 7.14 API 헬스체크 테스트 매트릭스

| 테스트 ID | 시나리오 | 예상 결과 | 우선순위 |
|-----------|----------|-----------|----------|
| TC-API-HEALTH-001 | 기본 헬스체크 | status: ok + 메타데이터 | Critical |
| TC-API-HEALTH-002 | 큐 카운트 | 3개 큐 카운트 반환 | High |
| TC-API-HEALTH-003 | Readiness 체크 | Redis ping 성공/실패 | Critical |
| TC-API-HEALTH-004 | Liveness 체크 | status: alive | High |
| TC-API-HEALTH-005 | 빈 큐 처리 | 에러 없음 | Medium |
| TC-API-HEALTH-006 | 대량 카운트 | 에러 없음 | Medium |
...
| 1.3 | 2025-12-28 | 워커 헬스체크 테스트 섹션 추가 (기본 응답/큐 상태/Readiness/Liveness) |
| 1.4 | 2025-12-29 | API 헬스체크 테스트 섹션 추가 (워커와 동일 구조) |
